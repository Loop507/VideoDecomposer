...
# --- 1. IMPORTAZIONE ROBUSTA PER MOVIEPY 2.x ---
try:
...
def prepare_clip(path, aspect):
    ...
    def frame_transform(get_frame, t):
        # Loop del tempo se il video è più corto della durata totale
...
def render_engine(video_paths, p):
    ...
    # --- FIX MOVIEPY 2.x PER FPS E AUDIO ---
    final = VideoClip(make_frame, duration=duration)
    final.fps = fps # Impostazione corretta attributo in v2.x

    if 0 in clips and clips[0].audio:
        try:
            # Sincronizzazione audio corretta per v2.x
            audio_track = clips[0].audio.with_duration(duration)
            final.audio = audio_track
        except:
            pass

    out_p = os.path.join(tempfile.gettempdir(), f"render_{int(time.time())}.mp4")
    
    # Scrittura finale con parametri espliciti per evitare errori di attributi
...
def main():
    st.set_page_config(layout="wide", page_title="Glitch Engine V3.4")
        ...
        if len(paths) < 1:
            ...
            with st.spinner("Rendering in corso..."):
                try:
                    ...
                    st.video(res)
                    st.download_button("📥 Scarica", open(res, "rb"), "glitch_final.mp4")
